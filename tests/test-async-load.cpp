...
static void syncQueues(IDevice* device, ICommandQueue* srcQueue, ICommandQueue* dstQueue)
{
    ...
    REQUIRE_CALL(dstQueue->submit(waitDesc));
    REQUIRE_CALL(dstQueue->waitOnHost());
}
...
GPU_TEST_CASE("async-load-texture", D3D12 | Vulkan | Metal)
{
    ...
    Size rowAlignment = 0;
    REQUIRE_CALL(device->getTextureRowAlignment(Format::RGBA8Unorm, &rowAlignment));
    const Size rowPitch = ((width * texelSize + rowAlignment - 1) / rowAlignment) * rowAlignment;
    const Size totalSize = rowPitch * height;

    // Generate test pattern: each pixel = (x*16, y*16, 128, 255)
    std::vector<uint8_t> texelData(totalSize);
    for (uint32_t y = 0; y < height; ++y)
    {
        for (uint32_t x = 0; x < width; ++x)
        {
            uint8_t* pixel = texelData.data() + (y * rowPitch + x * texelSize);
    ...
    // D3D12 textures cross COPY queue boundaries in COMMON; Vulkan needs a paired ownership transfer.
    TextureDesc texDesc = {};
    ...
    texDesc.defaultState = device->getInfo().deviceType == DeviceType::D3D12
        ? ResourceState::General : ResourceState::CopyDestination;
    ...
    ComPtr<IBuffer> stagingBuffer;
    REQUIRE_CALL(device->createBuffer(stagingDesc, texelData.data(), stagingBuffer.writeRef()));

    {
        auto enc = graphicsQueue->createCommandEncoder();
        enc->releaseTextureForQueue(gpuTexture, kEntireTexture, texDesc.defaultState, QueueType::Transfer);
        REQUIRE_CALL(graphicsQueue->submit(enc->finish()));
    }
...

    {
        auto enc = transferQueue->createCommandEncoder();
        enc->setBufferState(stagingBuffer, ResourceState::CopySource);
        enc->acquireTextureFromQueue(gpuTexture, kEntireTexture, texDesc.defaultState, QueueType::Graphics);
